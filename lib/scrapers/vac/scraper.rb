class Scrapers::Vac::Scraper < Scrapers::ContractScraper
  extend Scrapers::UrlHelpers
  ...
  def scrape_contract(url)
    ...
    Scrapers::DlExtractor.new(url, mappings).extract
  end
  ...
  def self.reports
    url = "http://www.veterans.gc.ca/eng/about-us/proactive-disclosure/contracts-over-10k/by-quarter/vac"
    page = Nokogiri::HTML(open(url))
    links = page.css('main[role="main"] ul li a[title~="View"]')
    links = page.xpath('//a[starts-with(@title, "View report for")]')
    links.reverse.map {|a_tag| Scrapers::Report.new("vac", full_url(page, url, a_tag["href"])) }
  end
end
