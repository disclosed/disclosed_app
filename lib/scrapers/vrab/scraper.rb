class Scrapers::Vrab::Scraper < Scrapers::ContractScraper
  BASE_URL = "http://www.vrab-tacra.gc.ca/Proactive-Divulgation"
  ...
  def contract_urls
    ...
    report_base_url = report.url.gsub(/\/[^\/]+\Z/, '')
    page.css("table.tableBorder a").map {|a_tag| "#{report_base_url}/#{a_tag['href']}" }
  end
  ...
  def self.reports
    Scrapers::ReportUrlExtractor.new("#{BASE_URL}/Contract-quarters-contrat-trimestres-eng.cfm", "vrab", "Veterans Review and Appeal Board Contracts", "h1").reports
  end
end
