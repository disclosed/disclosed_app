class Scrapers::Stat::Scraper < Scrapers::ContractScraper
  ...
  def scrape_contract(url)
    mappings = [
      Scrapers::TableMapping.new(:vendor_name, "vendor"),
  ...
  def contract_urls
    Scrapers::ContractUrlExtractor.new(report.url, "Vendor").urls
  end
  ...
  def self.reports
    Scrapers::ReportUrlExtractor.new("http://www.statcan.gc.ca/eng/about/contract/report", "stat", "Fiscal Year 2015/16", "h2").reports
  end
end
