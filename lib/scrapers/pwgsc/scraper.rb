class Scrapers::Pwgsc::Scraper < Scrapers::ContractScraper
  ...
  def scrape_contract(url)
    ...
    contract[:raw_contract_period] = extractor.page.xpath('//tr[regex(., "Contract Period - From")]//td[regex(., "^(?!.*Contract Period - From).*$")]', NokogiriXpathExtensions.new).text
    contract[:raw_contract_period] << " to "
    contract[:raw_contract_period] << extractor.page.xpath('//tr[regex(., "Contract Period - To")]//td[regex(., "^(?!.*Contract Period - To).*$")]', NokogiriXpathExtensions.new).text
    contract
  end
  ...
  def contract_urls
    Scrapers::ContractUrlExtractor.new(report.url).urls[4..-1]
  end
  ...
  def self.reports
    Scrapers::ReportUrlExtractor.new("http://www.tpsgc-pwgsc.gc.ca/cgi-bin/proactive/cl.pl?lang=eng&SCR=Q&Sort=0", "pwgsc", "Reports", "h1").reports
  end
end
