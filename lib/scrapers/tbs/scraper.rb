class Scrapers::Tbs::Scraper < Scrapers::ContractScraper
  BASE_URL = "http://www.tbs-sct.gc.ca/scripts/contracts-contrats"
  ...
  def contract_urls
    ...
    page.xpath("//table[contains(., 'Vendor Name')]//a").map {|a_tag| "#{BASE_URL}/#{a_tag['href']}" }
  end
  ...
  def self.reports
    Scrapers::ReportUrlExtractor.new("#{BASE_URL}/reports-rapports-eng.asp", "tbs", "Disclosure of Contracts Over $10,000 - Reports by Quarter", "h1").reports
  end
end
